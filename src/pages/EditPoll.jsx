...
const EditPoll = () => {
  ...
  const loadPoll = async () => {
    try {
      ...
      setFormData({
        ...
        tags: poll.tags ? poll.tags.join(', ') : '',
      });
    } catch (err) {
      const errorMsg = 'Failed to load poll. Please try again.';
      setError(errorMsg);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMsg = err.message || 'Failed to update poll. Please try again.';
      setError(errorMsg);
      showToast.error(errorMsg);
    } finally {
      setSaving(false);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                  ...
                  <div key={index} className="flex items-center space-x-2">
                    ...
                    {formData.options.length > 2 && (
                      <button
                        ...
                        className="px-3 py-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition focus:outline-none focus:ring-2 focus:ring-red-500"
              ...
              {formData.options.length < 10 && (
                <button
                  type="button"
                  onClick={addOption}
                  className="mt-3 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-sm font-medium flex items-center focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded px-2 py-1"
            ...
            <div>
              <label htmlFor="tags" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Tags <span className="text-gray-500 dark:text-gray-400 font-normal">(Optional, comma-separated)</span>
              </label>
              <input
                type="text"
                id="tags"
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 pt-4">
              ...
              <Button
                ...
                aria-busy={saving}
                aria-label={saving ? 'Saving poll' : 'Save poll changes'}
              >
                {saving ? (
                  <>
